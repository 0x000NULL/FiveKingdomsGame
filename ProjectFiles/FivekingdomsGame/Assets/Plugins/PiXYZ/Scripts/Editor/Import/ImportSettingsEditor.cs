...
namespace PiXYZ.Import.Editor {
    ...
    public class ImportSettingsEditor : UnityEditor.Editor {
        ...
        private static void Register() {
            ...
            templatePC.hasLODs.status = ParameterAvailability.Hidden;
            templatePC.hasLODs.defaultValue = false;
            templatePC.lodsMode.status = ParameterAvailability.Hidden;
            templatePC.qualities.status = ParameterAvailability.Hidden;
            // Rendering
        ...
        public bool drawSetting<T>(ref T value, Parameter<T> setting, int indent = 0) {
            ...
            if (setting.status == ParameterAvailability.Hidden) return false;
            ...
            if (template.splitTo16BytesIndex.status != ParameterAvailability.Hidden
             ...
             || template.qualities.status != ParameterAvailability.Hidden) {
                EditorGUIExtensions.DrawSubCategory(Style, "GEOMETRY", () => {
                    if (template.voxelizeGridSize.status == ParameterAvailability.Available) {
                        importSettings.voxelizePointCloud = EditorGUILayout.Toggle(new GUIContent("Split", template.voxelizeGridSize.tooltip), importSettings.voxelizePointCloud);
                        if (importSettings.voxelizePointCloud) {
                            importSettings.voxelizeGridSize = Mathf.Clamp(importSettings.voxelizeGridSize, 1, 100);
                            drawSetting(ref importSettings.voxelizeGridSize, template.voxelizeGridSize);
                        }
                    }
                    ...
                    if ((template.hasLODs.status == ParameterAvailability.Available) ? importSettings.hasLODs : template.hasLODs.defaultValue) {
                        ...
                        drawSetting(ref meshQuality, new Parameter<MeshQuality> { name = "Mesh Quality", status = template.qualities.status, tooltip = ImportSettings.TOOLTIP_QUALITY_PRESET });
