...
namespace PiXYZ.Tools.Editor {
    ...
    public sealed class RuleSetEditor : UnityEditor.Editor {
        ...
        private void drawRuleBlockGUI(RuleBlock block) {
            ...
            GUILayout.Label(new GUIContent(block.action.displayNameRuleEngine, block.action.tooltip), style.ruleBlockTitle);
            ...
            if (replaceSource != null) {
                ...
                if (mouseY > rect.y && mouseY < rect.y + rect.height) {
                    if (replaceSource != block) {
                        ...
                        GUI.Box(new Rect(rect.x, (replaceSourceY < replaceDestinationY) ? replaceSourceY : replaceDestinationY, 16, Mathf.Abs(replaceDestinationY - replaceSourceY)), "", canReorderHere ? StaticStyles.DragDropReplaceValid : StaticStyles.DragDropReplaceInvalid);
            ...
            if (errors?.Length > 0) {
                GUI.Box(new Rect(rect.x + rect.width - 55, rect.y + 7, 24, 24), ToGUIContent(errors), StaticStyles.IconErrorSmall);
                GUI.Label(new Rect(rect.width - 32, rect.y + 11, 24, 24), errors.Length.ToString(), style.ruleBlockText);
            ...
            if (warnings?.Length > 0) {
                GUI.Box(new Rect(rect.x + rect.width - 55 + shift, rect.y + 7, 24, 24), ToGUIContent(warnings), StaticStyles.IconWarningSmall);
                GUI.Label(new Rect(rect.width - 32 + shift, rect.y + 11, 24, 24), warnings.Length.ToString(), style.ruleBlockText);
                shift -= 30;
            }

            if (infos?.Length > 0) {
                GUI.Box(new Rect(rect.x + rect.width - 55 + shift, rect.y + 7, 24, 24), ToGUIContent(infos), StaticStyles.IconInfoSmall);
                GUI.Label(new Rect(rect.width - 32 + shift, rect.y + 11, 24, 24), infos.Length.ToString(), style.ruleBlockText);
            }
        ...
        public static GUIContent ToGUIContent(string[] messages) {
