...
namespace PiXYZ.Tools.Editor {
    ...
    public sealed class CoreContext : IDisposable {
        ...
        public CoreContext(GameObject[] gameObjects, bool manageTextures) {
            ...
            var settings = ScriptableObject.CreateInstance<ImportSettings>();
            settings.isZUp = false;
            var sceneExtractConverter = new UnityToSceneExtract(gameObjects, settings, manageTextures);
            var sceneExtract = sceneExtractConverter.sceneExtract;
        ...
        public void Dispose() {
            ...
            for (int treeIndex = 0; treeIndex < newScene.tree.length; ++treeIndex) {
                ...
                if (meshIdToMesh.ContainsKey(meshId)) {
                    ...
                    Dispatcher.StartCoroutine(meshConverter.Convert(newScene.meshes[meshId], true, false, 0.001f, null, false));
                ...
                if (renderer) {
                    ...
                    if (filter) {
                        if (meshConverter.mesh) {
                            meshConverter.mesh.name = (filter.sharedMesh == null) ? "mesh" : filter.sharedMesh.name;
