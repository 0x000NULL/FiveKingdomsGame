...
namespace PiXYZ.Interface
{
    ...
    public static class InterfaceConversions
    {
        ...
        public static NativeInterface.ImportSettings ToInterfaceObject(this ImportSettings settings, ImportSettingsTemplate template)
        {
            ...
            nsettings.voxelizeGridSize = settings.voxelizePointCloud ? (template.voxelizeGridSize.status == ParameterAvailability.Available) ? settings.voxelizeGridSize : template.voxelizeGridSize.defaultValue : 0;
            ...
            nsettings.meshQualities = ((template.qualities.status == ParameterAvailability.Available) ? (settings.hasLODs ? settings.qualities : new LodsGenerationSettings(new LodGenerationSettings[] { settings.qualities.quality } )) : template.qualities.defaultValue).ToInterfaceObject();
        ...
        #region LODs
        public static NativeInterface.MeshQualityList ToInterfaceObject(this LodsGenerationSettings unityObject)
        {
            var iLods = new List<MeshQuality>();
            for (int l = 0; l < unityObject.lods.Length; l++) {
                if (/*lods[l].quality == LODQuality.BILLBOARD || */unityObject.lods[l].quality == LodQuality.CULLED)
                    continue;
                iLods.Add((MeshQuality)Enum.Parse(typeof(MeshQuality), unityObject.lods[l].quality.ToString()));
            }
            return new NativeInterface.MeshQualityList(iLods.ToArray());
        }

        #endregion

        #region Meshes
        public static NativeInterface.MeshDefinition ToInterfaceObject(this Mesh unityObject, NativeInterface.MaterialDefinition[] materials, bool mirrorX, bool isZup, float scale)
        {
            MeshToMeshDefinition mc = new MeshToMeshDefinition();
            Dispatcher.StartCoroutine(mc.Convert(unityObject, materials, mirrorX, isZup, scale));
            return mc.meshDefinition;
        }

        public static Mesh ToUnityObject(this NativeInterface.MeshDefinition interfaceObject, bool mirrorX, bool isZup, float scale)
        {
            MeshDefinitionToMesh mc = new MeshDefinitionToMesh();
            Dispatcher.StartCoroutine(mc.Convert(interfaceObject, mirrorX, isZup, scale));
            return mc.mesh;
        }
        #endregion
        ...
        public static Material ToUnityObject(this NativeInterface.MaterialDefinition interfaceObject, Shader shader, List<Texture2D> textures)
        {
            ...
            float alpha = (interfaceObject.opacity._type == NativeInterface.CoeffOrTexture.Type.COEFF ? (float)interfaceObject.opacity.coeff : 1f);
            if (alpha < 1f) {
        ...
        internal static Texture2D ConvertToNormalMap(this Texture2D texture)
        {
            ...
            for (int i = 0; i < colors.Length; i++) {
                ...
                // 0.975f is kind of a "Magic Number" that goes well with shader tricks used in the Standard Shader
        ...
        public static Matrix4x4 GetMatrix(this Transform transform, bool localMatrix, bool mirrorX, bool isZup, float scaleFactor)
        {
            ...
            if (mirrorX) rotation = new Quaternion(rotation.x, -rotation.y, -rotation.z, rotation.w);
            if (isZup) rotation = new Quaternion(transform.localRotation.x, -rotation.z, rotation.y, rotation.w);

            translation = new Vector3(
                mirrorX ? -translation.x : translation.x,
                isZup ? -translation.z : translation.y,
                isZup ? translation.y : translation.z) / scaleFactor;
        ...
        public static NativeInterface.Matrix4 ToInterfaceObject(this Transform transform, bool localMatrix, bool mirrorX, bool isZup, float scaleFactor)
        {
            return transform.GetMatrix(localMatrix, mirrorX, isZup, scaleFactor).ToInterfaceObject();
        }
        #endregion
        ...
        public static string ToEndDateRichText(this NativeInterface.Date date)
        {
            ...
            return date.ToUnityObject().ToString("yyyy-MM-dd") + "   (<color='" + remainingTextColor + "'><b>" + daysRemaining + "</b> Day" + (daysRemaining > 1 ? "s" : "") + " remaining</color>)";
        }
        #endregion
