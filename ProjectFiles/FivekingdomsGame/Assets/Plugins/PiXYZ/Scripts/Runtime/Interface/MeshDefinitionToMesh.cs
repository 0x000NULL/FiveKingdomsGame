...
namespace PiXYZ.Interface {
    ...
    public sealed class MeshDefinitionToMesh {
        ...
        public IEnumerator Convert(NativeInterface.MeshDefinition meshDefinition, bool mirrorX, bool isZUp, float scale, MeshDefinitionToMeshHandler conversionCallback = null, bool isAsynchronous = false) {
            ...
            try {
                ...
                for (int s = 0; s < triangles.Length; s++) {
                    ...
                    for (int t = 0; t < trcount; t++) {
                        if (!mirrorX || t % 3 == 0)
                            striangles[t] = meshDefinition.triangles[offset + t];
                        else if (t % 3 == 1)
                            striangles[t] = meshDefinition.triangles[offset + t + 1];
                        else if (t % 3 == 2)
                            striangles[t] = meshDefinition.triangles[offset + t - 1];
                    }
                }
